...
const getTopItemsByCategory = (orders) => {
  ...
  categoryMap.forEach((items, category) => {
    ...
    const sortedItems = Array.from(items.values()).sort((a, b) => b.totalQuantity - a.totalQuantity);
...
const getWeeklyTotalSales = (orders) => {
  ...
  orders.forEach(order => {
    ...
    if (orderMonth === currentMonth && orderYear === currentYear) {
      if (!weeklySalesMap.has(weekRange)) {
        weeklySalesMap.set(weekRange, 0);
      }
      weeklySalesMap.set(weekRange, weeklySalesMap.get(weekRange) + parseFloat(order.total_bill));
    }
  });

  const weeklySales = Array.from(weeklySalesMap.entries()).map(([week, sales]) => ({
    week,
    sales
  }));
...
const getwaiterdetails=async(resid)=>{
  ...
  if(res.data.success){
    ...
    const presentwaiters = res.data?.data.filter(
      (waiter) =>
        waiter.ispresent === true 
    );
    const absentwaiters = res.data?.data.filter(
      (waiter) =>
        waiter.ispresent === false 
    );
