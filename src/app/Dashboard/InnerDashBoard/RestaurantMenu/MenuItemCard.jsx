  ...
  const handleSwitchChange = async (event) => {
    if (
      ...
      try {
        ...
        console.error(
          "Error occurred while updating food item status:",
          error.message
        );
      } finally {
        setLoading(false);
  ...
  return (
    <div className="bg-white drop-shadow-lg shadow-slate-400 rounded-lg border min-h-24 w-[250px] p-2">
      <div className="grid grid-cols-4 justify-items-stretch items-center my-2">
        <h3
          className="font-semibold capitalize text-base text-zinc-800 col-span-3 truncate"
          title={item.name}
        >
          {item.name}
        </h3>
      ...
      <div className="mt-1 flex justify-between items-center">
        ...
        <div className="relative flex items-center">
          <Switch
            isChecked={item.available_status}
