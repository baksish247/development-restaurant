...
const AddNewItem = ({ onClose,fetchitems, currentInventoryItems , restaurant_id}) => {
  ...
  const filteredItems = globalItems
    ...
    .filter(globalItem => 
      !currentInventoryItems.some(currentItem => currentItem.item_name === globalItem.item_name)
    );
  ...
  return (
    <div className="fixed z-50 h-screen w-screen top-0 left-0 flex justify-center overflow-x-auto items-center bg-black/20 backdrop-blur-sm">
      <Toaster/><div className="bg-white p-6 rounded-md shadow-md max-w-2xl w-full mx-auto relative">
        <button
          onClick={onClose}
          ...
          <div className="w-1/2 p-4 border-r border-gray-200">
            ...
            <ul className="max-h-96 overflow-y-auto">
              {filteredItems.map(item => (
                ...
                >
                  <span className='text-left col-span-1' >{item.item_photo}</span>
          ...
          <div className="w-1/2 p-4">
            {selectedItem ? (
              <form onSubmit={formik.handleSubmit}>
                <div className="mb-4">
                  <h2 className="text-lg font-semibold mb-2">{selectedItem.item_name} &nbsp;&nbsp;{selectedItem.item_photo}</h2>
                  <p className='text-sm'>Code: {selectedItem.item_code}</p>
                  <p className='text-sm'>Group: {selectedItem.item_group}</p>
                  <p className='text-sm'>Type: {selectedItem.type === "Measurable" ? "Can be measured per dish" : "Cannot be measured per dish"}</p>
                </div>

                <div className="mb-4">
                  <label className="block text-gray-700 font-semibold mb-2">Amount to add in stock (in kgs):</label>
                  <input
                    type="number"
                    name="amount"
