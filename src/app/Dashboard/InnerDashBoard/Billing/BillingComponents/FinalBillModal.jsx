      ...
      <div class="invoice-container">
          <div class="invoice">
              ...
              <table class="invoice-details">
                  <tbody>
                      <tr>
                          <td class="text-sm">Bill No.: ${orderbill?.bill_no}</td>
                          <td class="text-sm text-right">Date: ${day}/${month}/${year}</td>
                      </tr>
                      <tr>
                          <td class="text-sm">Table No: ${selectedOrder.table_number}</td>
                      ...
                      ${selectedOrder.order_items.map((orderitems, j) => 
                        orderitems.items.map((item, k) => 
                          `<tr key="${k}">
                            ...
                            <td class="text-sm text-right">${parseFloat(item.quantity).toFixed(2)}</td>
                            <td class="text-sm text-right">${parseFloat(item.food.price).toFixed(2)}</td>
                            <td class="text-sm text-right">${(parseFloat(item.quantity) * parseFloat(item.food.price)).toFixed(2)}</td>
                          </tr>`
                        ).join("")
                      ).join("")}
                  </tbody>
              ...
              <table class="summary-table">
                  <tbody>
                      <tr>
                          ...
                          <td class="text-sm text-right">${parseFloat(selectedOrder.initial_bill).toFixed(2)}</td>
                      </tr>
                      ${parseFloat(orderbill?.discountamount) > 0.0 ? `
                        <tr>
                            <td class="text-sm">DISCOUNT @ (${orderbill.discountpercent}%)</td>
                            <td class="text-sm text-right">${parseFloat(orderbill?.discountamount).toFixed(2)}</td>
                        </tr>
                        <tr>
                            <td class="text-sm">NET AMOUNT</td>
                            <td class="text-sm text-right">${(parseFloat(selectedOrder.initial_bill) - parseFloat(orderbill?.discountamount)).toFixed(2)}</td>
                        </tr>` : ""
                      }
                      <tr>
                          <td class="text-sm">CGST @ (${restaurantinfo.cgst}%)</td>
                          <td class="text-sm text-right">${orderbill?.cgstamount}</td>
                      </tr>
                      <tr>
                          <td class="text-sm">SGST @ (${restaurantinfo.sgst}%)</td>
                          <td class="text-sm text-right">${orderbill?.sgstamount}</td>
                      </tr>
                      <tr class="grand-total">
                          <td class="text-sm font-bold">GRAND TOTAL</td>
                          <td class="text-sm font-bold text-right">₹ ${orderbill?.total_bill}</td>
                      </tr>
                  </tbody>
              </table>
    ...
    <>
      {orderbill && (
        <div className="lg:mx-4 overflow-hidden">
          ...
          <div
            className="bg-white p-2   shadow-md border-2 border-gray-300"
          >
